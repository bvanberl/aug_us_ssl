...
class SimCLRLoss(nn.Module):
    ...
    def forward(
        ...
        contrastive_accuracy = multiclass_accuracy(
            torch.cat([logits1, logits2], dim=0),
            torch.cat([labels, labels], dim=0)
        )

        loggables = {"contrastive_accuracy": contrastive_accuracy}

        return loss, loggables
...
class BarlowTwinsLoss(nn.Module):
    ...
    def forward(
        ...
        loggables = {"inv_term": inv_term, "rr_term": rr_term}
...
class VICRegLoss(nn.Module):
    ...
    def forward(
        ...
        # Calculate invariance term
        inv_term = self._invariance_term(z1, z2, sw=sw)
        ...
        var_term = self._variance_term(z1, z2)
